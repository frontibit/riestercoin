...
uint256 hashGenesisBlock("0xd480a562bb56a0759a8143eafb4dddf8660ef1a154e567e0b69de4f9356b2528");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Riestercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Riestercoin Signed Message:\n";
...
{
    // Riestercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Riestercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // Riestercoin: 3.5 days
static const int64 nTargetSpacing = 60; // Riestercoin: 60 Sec
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Riestercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Riestercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Riestercoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Yeah Riester is timeless";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x89d5ca1489411085dd79968cd36309e7310135e228244757dc2f1343f4360df9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Riestercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RiestercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RiestercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RiestercoinMiner(CWallet *pwallet)
{
    printf("RiestercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("riestercoin-miner");
    ...
    try { loop {
        ...
        printf("Running RiestercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RiestercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RiestercoinMiner, pwallet));
}
